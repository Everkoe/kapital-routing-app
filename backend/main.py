...
async def register_user(usuario: UsuarioRegistro):
    ...
    # En un sistema real, la contraseña se almacenaría hasheada.
    # Ejemplo: from passlib.context import CryptContext; pwd_context = CryptContext(schemes=["bcrypt"], deprecated="auto"); hashed_password = pwd_context.hash(usuario.password)
    usuarios_db[usuario.email] = {
        "email": usuario.email,
        "password": usuario.password, # Simulación: guardar en texto plano
